...
LANGUAGE_CONFIG = {
    ...
    ".rs": {
        ...
        "symbols": {"function_item", "struct_item", "enum_item", "trait_item", "impl_item"},
    },
    ".c": {
        "language": "c",
...
def chunk_codebase(repo_path):
    if not os.path.isdir(repo_path):
        print(f"Skipping code chunking: Repository not found at {repo_path}")
        return []
    ...
    for root, dirs, files in os.walk(repo_root):
        ...
        for filename in files:
            ...
            chunks.extend(chunk_file(file_path, repo_root, config))
...
def chunk_file(file_path, repo_root, config):
    relative_path = to_relative_path(file_path, repo_root)
    language = config["language"]

    try:
        source_bytes = read_file_bytes(file_path)
        semantic_chunks = extract_semantic_chunks(source_bytes, relative_path, language, config)
        if semantic_chunks:
            return semantic_chunks
    except Exception as error:
        print(f"Tree-sitter failed for {relative_path}: {error}")

    return extract_fallback_chunks(file_path, relative_path, language)


def extract_semantic_chunks(source_bytes, relative_path, language, config):
    parser = Parser(Language(config["grammar"]()))
    tree = parser.parse(source_bytes)

    chunks = []

    def visit(node):
        if node.type in config["symbols"]:
...
def format_code_chunk(node, source_bytes, relative_path, language):
...
def extract_fallback_chunks(file_path, relative_path, language):
...
def get_symbol_name(node, source_bytes):
    ...
    if name_node:
        return decode_source(source_bytes[name_node.start_byte : name_node.end_byte]).strip()
...
def collect_identifier_nodes(node, matches):
...
def is_function_like(node_type):
    return node_type in {
        "function_definition",
        "function_declaration",
        "function_item",
...
def to_relative_path(file_path, repo_root):
